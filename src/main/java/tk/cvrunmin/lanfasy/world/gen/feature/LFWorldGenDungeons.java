...
public class LFWorldGenDungeons extends WorldGenerator
{
    private static final Logger field_175918_a = LogManager.getLogger();
    private static final String[] SPAWNERTYPES = new String[] {"LFSkeleton", "LFZombie", "LFZombie", "LFSpider"};
                ...
                for (i2 = 3; i2 >= -1; --i2)
                {
                    for (j2 = i1; j2 <= j1; ++j2)
                    {
                        ...
                        else if (worldIn.getBlockState(blockpos1).getBlock().getMaterial().isSolid() && worldIn.getBlockState(blockpos1).getBlock() != Blocks.chest)
                        {
                            ...
                            {
                                worldIn.setBlockState(blockpos1, LFBlocks.textureless.getDefaultState(), 2);
                            }
                            else
                            {
                                worldIn.setBlockState(blockpos1, LFBlocks.lightless.getDefaultState(), 2);
                            }
                        }
                    }
                }
    ...
    private String pickMobSpawner(Random p_76543_1_)
    {
    	...
    	if(DungeonHooks.getRandomDungeonMob(p_76543_1_).equals("Skeleton")){
    		spawntype = "LFSkeleton";
    	}
    	else if(DungeonHooks.getRandomDungeonMob(p_76543_1_).equals("Zombie")){
    		spawntype = "LFZombie";
    	}
    	else if(DungeonHooks.getRandomDungeonMob(p_76543_1_).equals("Spider")){
    		spawntype = "LFSpider";
    	}
    	else{
    		spawntype = "LFZombie";
