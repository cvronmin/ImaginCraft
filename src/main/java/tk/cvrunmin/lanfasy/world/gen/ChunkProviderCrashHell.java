...
import tk.cvrunmin.lanfasy.world.gen.feature.WorldGenCrashLava;

public class ChunkProviderCrashHell implements IChunkProvider{
    ...
    private final WorldGenCrashLava field_177473_x;
    private final WorldGenCrashLava field_177472_y;
    ...
    public ChunkProviderCrashHell(World worldIn, boolean p_i45637_2_, long p_i45637_3_)
    {
        this.field_177467_w = new WorldGenMinable(Blocks.quartz_ore.getDefaultState(), 14, BlockHelper.forBlock(LFBlocks.crash_netherrack));
        this.field_177473_x = new WorldGenCrashLava(Blocks.flowing_lava, true);
        this.field_177472_y = new WorldGenCrashLava(Blocks.flowing_lava, false);
                    ...
                    for (int l1 = 0; l1 < 8; ++l1)
                    {
                        ...
                        for (int i2 = 0; i2 < 4; ++i2)
                        {
                            ...
                            for (int j2 = 0; j2 < 4; ++j2)
                            {
                                ...
                                if (d15 > 0.0D)
                                {
                                    iblockstate = LFBlocks.crash_netherrack.getDefaultState();
                                }
    ...
    public void func_180516_b(int p_180516_1_, int p_180516_2_, ChunkPrimer p_180516_3_)
    {
        ...
        for (int k = 0; k < 16; ++k)
        {
            for (int l = 0; l < 16; ++l)
            {
                ...
                int j1 = -1;
                IBlockState iblockstate = LFBlocks.crash_netherrack.getDefaultState();
                IBlockState iblockstate1 = LFBlocks.crash_netherrack.getDefaultState();
                ...
                for (int k1 = 127; k1 >= 0; --k1)
                {
                    if (k1 < 127 - this.hellRNG.nextInt(5) && k1 > this.hellRNG.nextInt(5))
                    {
                        ...
                        {
                            if (iblockstate2.getBlock() == LFBlocks.crash_netherrack)
                            {
                                if (j1 == -1)
                                {
                                    if (i1 <= 0)
                                    {
                                        iblockstate = null;
                                        iblockstate1 = LFBlocks.crash_netherrack.getDefaultState();
                                    }
