...
public class TeleporterCrashHell extends Teleporter{
    ...
    public TeleporterCrashHell(WorldServer worldIn)
    {
    	super(worldIn);
    ...
    public boolean placeInExistingPortal(Entity entityIn, float p_180620_2_)
    {
        ...
        {
            TeleporterCrashHell.PortalPosition portalposition = (TeleporterCrashHell.PortalPosition)this.destinationCoordinateCache.getValueByKey(k);
            d0 = 0.0D;
            ...
            for (int l = -128; l <= 128; ++l)
            {
                ...
                for (int i1 = -128; i1 <= 128; ++i1)
                {
                    for (BlockPos blockpos = blockpos4.add(l, this.worldServerInstance.getActualHeight() - 1 - blockpos4.getY(), i1); blockpos.getY() >= 0; blockpos = blockpos1)
                    {
                        ...
                        if (this.worldServerInstance.getBlockState(blockpos).getBlock() == LFBlocks.portal_crashhell)
                        {
                            while (this.worldServerInstance.getBlockState(blockpos1 = blockpos.down()).getBlock() == LFBlocks.portal_crashhell)
                            {
                                blockpos = blockpos1;
                            }
        ...
        if (d0 >= 0.0D)
        {
            if (flag1)
            {
                this.destinationCoordinateCache.add(k, new TeleporterCrashHell.PortalPosition((BlockPos)object, this.worldServerInstance.getTotalWorldTime()));
            ...
            if (this.worldServerInstance.getBlockState(((BlockPos)object).west()).getBlock() == LFBlocks.portal_crashhell)
            {
                enumfacing = EnumFacing.NORTH;
            }

            if (this.worldServerInstance.getBlockState(((BlockPos)object).east()).getBlock() == LFBlocks.portal_crashhell)
            {
                enumfacing = EnumFacing.SOUTH;
            }

            if (this.worldServerInstance.getBlockState(((BlockPos)object).north()).getBlock() == LFBlocks.portal_crashhell)
            {
                enumfacing = EnumFacing.EAST;
            }

            if (this.worldServerInstance.getBlockState(((BlockPos)object).south()).getBlock() == LFBlocks.portal_crashhell)
            {
    ...
    public boolean makePortal(Entity p_85188_1_)
    {
        ...
        IBlockState iblockstate = LFBlocks.portal_crashhell.getDefaultState().withProperty(BlockLFPortalHell.AXIS, l5 != 0 ? EnumFacing.Axis.X : EnumFacing.Axis.Z);
    ...
    public void removeStalePortalLocations(long p_85189_1_)
    {
        if (p_85189_1_ % 100L == 0L)
        {
            ...
            while (iterator.hasNext())
            {
                ...
                TeleporterCrashHell.PortalPosition portalposition = (TeleporterCrashHell.PortalPosition)this.destinationCoordinateCache.getValueByKey(olong.longValue());
