...
public class EntityCorn extends EntityMess implements IBossDisplayData {
	...
	public EntityCorn(World worldIn) {
		super(worldIn);
    ...
    protected void onAttackMode(){
    	if (!worldObj.isRemote) {
			if (rand.nextInt(100) < 2) {
				worldObj.createExplosion(this, this.posX + rand.nextInt(10)-4, this.posY + rand.nextInt(10)-4, this.posZ + rand.nextInt(10)-4, rand.nextInt(), true);
			}
		}
    }
...
/*            if (entitylivingbase != null && this.worldObj.getDifficulty() == EnumDifficulty.HARD && (double)this.rand.nextFloat() < this.getEntityAttribute(reinforcementChance).getAttributeValue()) {
    ...
    protected void dropFewItems(boolean p_70628_1_, int p_70628_2_)
    {
        if (p_70628_1_)
        {
            ...
            for (int k = 0; k < j; ++k)
            {
                this.entityDropItem(new ItemStack(LFHecItems.macrystal, 1, 8), 0f);
            }
        }
    }
}
