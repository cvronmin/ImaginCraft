...
public class EntityHammer extends EntityMess implements IBossDisplayData {
	...
	public EntityHammer(World worldIn) {
		super(worldIn);
...
/*            if (entitylivingbase != null && this.worldObj.getDifficulty() == EnumDifficulty.HARD && (double)this.rand.nextFloat() < this.getEntityAttribute(reinforcementChance).getAttributeValue()) {
    ...
    {
    	entityIn.addVelocity((double)(-MathHelper.sin(this.rotationYaw * (float)Math.PI / 180.0F) * (float)5F), 0.1D, (double)(MathHelper.cos(this.rotationYaw * (float)Math.PI / 180.0F) * (float)5F));
    ...
    protected void dropFewItems(boolean p_70628_1_, int p_70628_2_)
    {
        if (p_70628_1_)
        {
            ...
            for (int k = 0; k < j; ++k)
            {
                this.entityDropItem(new ItemStack(LFHecItems.macrystal, 1, 5), 0f);
            }
        }
    }
}
