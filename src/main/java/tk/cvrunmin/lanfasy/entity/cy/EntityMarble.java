...
public class EntityMarble extends EntityMess implements IBossDisplayData{
	...
	public EntityMarble(World worldIn) {
		super(worldIn);
...
/*            if (entitylivingbase != null && this.worldObj.getDifficulty() == EnumDifficulty.HARD && (double)this.rand.nextFloat() < this.getEntityAttribute(reinforcementChance).getAttributeValue()) {
    ...
    protected void dropFewItems(boolean p_70628_1_, int p_70628_2_)
    {
        if (p_70628_1_)
        {
            ...
            for (int k = 0; k < j; ++k)
            {
                this.entityDropItem(new ItemStack(LFHecItems.macrystal, 1, 3), 0f);
            }
        }
    }
}
