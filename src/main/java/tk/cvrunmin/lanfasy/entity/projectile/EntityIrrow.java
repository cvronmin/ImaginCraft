...
public class EntityIrrow extends EntitySafeIrrow implements IProjectile{
    private int xTile = -1;
    private int yTile = -1;
    private int zTile = -1;
    ...
    private int ticksExplosing;
    private double damage = 2.0D;
    private int knockbackStrength;
    private int affectedRadius = 5;

    public EntityIrrow(World worldIn)
    {
        super(worldIn);
    ...
    public EntityIrrow(World worldIn, double x, double y, double z)
    {
        super(worldIn);
        this.renderDistanceWeight = 10.0D;
        this.setSize(0.5F, 0.5F);
    ...
    public EntityIrrow(World worldIn, EntityLivingBase shooter, EntityLivingBase p_i1755_3_, float p_i1755_4_, float p_i1755_5_)
    {
        super(worldIn);
        this.renderDistanceWeight = 10.0D;
    ...
    public EntityIrrow(World worldIn, EntityLivingBase shooter, float p_i1756_3_)
    {
        super(worldIn);
        this.renderDistanceWeight = 10.0D;
        this.shootingEntity = shooter;
    ...
    public void onUpdate()
    {
        ...
        if(ticksExplosing > LFConfig.startIon){
            ...
            explosion1.doExplosionD(true);
        }
        if(ticksExplosing >= LFConfig.endIon){
        	this.setDead();
        }
        }
        if (this.inGround)
        ...
        else
        {
            ...
            if (movingobjectposition != null)
            {
                if (movingobjectposition.entityHit != null)
                {
                    ...
                    {
                        damagesource = DamageSourceLF.causeIrrowDamage(this, this);
                    }
                    else
                    {
                        damagesource = DamageSourceLF.causeIrrowDamage(this, this.shootingEntity);
                    }
    ...
    public void writeEntityToNBT(NBTTagCompound tagCompound)
    {
        ...
        tagCompound.setByte("AffectedRadius", (byte)this.affectedRadius);
    }
    ...
    public void readEntityFromNBT(NBTTagCompound tagCompund)
    {
        ...
        if (tagCompund.hasKey("AffectedRadius", 99))
        {
            this.affectedRadius = tagCompund.getByte("AffectedRadius");
        }
    }
    ...
    public void onCollideWithPlayer(EntityPlayer entityIn)
    {
        if (!this.worldObj.isRemote && this.inGround && this.arrowShake <= 0)
        {
            ...
            if (this.canBePickedUp == 1 && !entityIn.inventory.addItemStackToInventory(new ItemStack(LFItems.fire_arrow, 1)))
            {
                flag = false;
            }
