...
public class ItemFireBow extends MEItem{
    ...
    public ItemFireBow()
    {
        ...
        this.setUnlocalizedName("fire_bow");
        this.setRegisteredName("fire_bow");
    }
	public ModelResourceLocation getModel(ItemStack stack, EntityPlayer player,
			int useRemaining) {
		if ((player.getItemInUse() != null)
				&& (stack.getItem() == LFItems.fire_bow)) {
			int j = stack.getMaxItemUseDuration() - player.getItemInUseCount();

	        if (j >= 18)
	        {
	            return new ModelResourceLocation("lanfasy:fire_bow_pulling_2", "inventory");
	        }
	        else if (j > 13)
	        {
	        	return new ModelResourceLocation("lanfasy:fire_bow_pulling_1", "inventory");
	        }
	        else if (j > 0)
	        {
	        	return new ModelResourceLocation("lanfasy:fire_bow_pulling_0", "inventory");
	        }
		}
